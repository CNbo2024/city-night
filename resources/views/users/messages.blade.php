	...
	<div class="message-wrap">
		...
		<div class="{{$message->sender_id == Auth::id() ? 'message-list me' :'message-list'}}">
			<div class="msg pl-2 pr-2 pb-2 pt-2 mb-2" >
				<p class="m-0">
					<img class="img-40x40" style="border-radius: 50%" src="{{App\Models\User::find($message->sender_id)->profile_src ?? '/public/images/default-profile.png'}}" alt="">
					<b>{{App\Models\User::find($message->sender_id)->first_name??'Usuario'}} {{App\Models\User::find($message->sender_id)->last_name??'Eliminado'}}:</b>
					{{$message->message}}
				</p>
			</div>			
