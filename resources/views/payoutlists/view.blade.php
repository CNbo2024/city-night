			...
			<div class="container-fluid container-fluid-90">
				<div class="row">
						<div class="col-md-3 mt-5">
							<nav class="navbar-expand-lg navbar-light border rounded-3 ">
								<ul class="sv_profile_nav">
									<li class="nav-item">
										<a class="text-14 text-color secondary-text-color font-weight-700 text-color-hover" href="{{ url('users/payout-list') }}">{{trans('messages.sidenav.payouts')}}</a>
									</li>
									...
									<li class="nav-item">
										<a class="text-14 text-color text-color-hover" href="{{ url('users/auth-payout') }}">{{trans('Setup automatic payout requests')}}</a>
									</li>
					...
					<div class="col-md-12 mt-5">
						<h3 class="text-20 font-weight-600">{{trans('messages.account_preference.my_wallet_balance')}}
						
						<?php 
						    $aa = "&#x20B9;";
						    
						    $curr_id = $walletBalance->currency_id;
				            $currency = App\Models\Currency::where(['id' => $curr_id ])->first(); 
				            $code = $currency->code;
						?>

						    <?php if($walletBalance->balance < 0) { ?>
						        <span class="vbadge-success p-2 button-rounded-4"> {!! $currency->symbol.currency_fix($walletBalance->balance, $code) !!} </span>

						    <?php } else { ?>
						        <span class="Accepted p-2 button-rounded-4"> {!! $currency->symbol.currency_fix($walletBalance->balance, $code) !!} </span>
						    <?php } ?>
						</h3>
					</div>
			...
			if ($('#next_btn').prop('disabled')) {
					...
					if(item.type == 4) {
						$('#confirmDiv').html('<div class="row">'
										...
										+'<div class="row w-100">'
											+'<div class="col-md-12 text-right mt-4">'
												+'<button type="button" class="btn btn-outline-danger text-14 mt-2 pl-4 pr-4 mr-2"  id="back_btn"> {{trans('messages.listing_description.back')}} </button>'
