...
class CronController extends Controller
{
    ...
    {
        $wallets = Wallet::get();
        $min = Settings::where('name', 'min-amount-for-payout-request')->first();

        foreach ($wallets as $wallet) {
            $amount = DB::table('automatic_payouts')->where('user_id', $wallet->user_id)->first()->value;

            if ($amount->value) {
                if ($wallet->total >= $amount) {
                    $userId            = $wallet->user_id;
                    $payoutSetting     = PayoutSetting::find($amount->payment_method);
                    $walletMoney       = Wallet::where('user_id', $userId)->first();

                    $withdrawal                         = new Withdrawal;
                    $withdrawal->user_id                = $userId;
                    $withdrawal->currency_id            = 1;
                    $withdrawal->payout_id              = $amount->payment_method;
    ...
    public function check_booking_expired()
    {
		...
		$results1        = Bookings::where('accepted_at','<',$formatted_date)->where('status', 'Processing')->get();
    ...
    public function importDump()
    {
        try {
            ...
            DB::unprepared(file_get_contents('db/sangvish_dump.sql'));
