...
class EmailController extends Controller
{
    ...
    public function bookingCancellation($booking_id)
    {
        ...
        $svuser = User::where('id', $booking->user_id)->first();

        if(env('APP_MODE', '') != 'test'){
            if($emailConfig['driver']=='smtp' && $emailConfig['email_status']==1){
                    Mail::send('emails.booking_cancel_template', $data, function($message) use($user, $data,$subject,$content,$svuser) {
                    $message->to($svuser->email, $user->first_name)->subject($subject);
            });
            }
            else if($emailConfig['driver']=='sendmail'){
              $this->sendPhpEmail($data,$emailConfig);
    ...
    public function bookingCancellationguest($booking_id)
    {
        $emailSettings   = Settings::where('type','email')->get()->toArray();
        $emailConfig     = $this->helper->key_value('name','value',$emailSettings);
        $adminDetails    = Admin::where('status','active')->first();
        ...
        $emailTemplatefromDB = EmailTemplate::where([['temp_id',12],['lang_id', $this->getDefaultLanguage()],['type','email']])->first();
        if (!empty($emailTemplatefromDB->subject) && !empty($emailTemplatefromDB->body))
        {
            $subjectFromDB = $emailTemplatefromDB->subject;
            $bodyFromDB    = $emailTemplatefromDB->body;
    ...
    public function contact_user($data)
    {
        ...
        $mail->setFrom($adminDetails->email, $adminDetails->username);
        $mail->addReplyTo($adminDetails->email, $adminDetails->username);
        $mail->addAddress($adminDetails->email, $data['username']);
        ...
        $message = '<html>
                      <table border="1" style="border-collapse:collapse">
                        ...
                        <tr style="border: 1px solid #ddd;">
                          ...
                          <td style="padding:5px">'.$data['useremail'].'</td>
                        </tr>
                        <tr style="border: 1px solid #ddd;"> 
    ...
    public function disapprove_user($data)
    {
        $emailSettings   = Settings::where('type','email')->get()->toArray();
        $emailConfig     = $this->helper->key_value('name','value',$emailSettings);
        $adminDetails                = Admin::where('status','active')->first();
        ...
        $mail->addAddress($data['useremail'], $data['uname']);
        

        $mail->Subject   = "Your Document Disapproved";

        $message = '<html>
                      <h1>Your Document Disapproved</h1>
                      <h3>Reason</h3>
                      <p>'.$data['reason_for_disapprove'].'</p>
                    </html>';
