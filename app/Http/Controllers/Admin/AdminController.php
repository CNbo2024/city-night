    ...
    public function delete(Request $request)
    {
        if ( $request->id == 1 ) {
           ...
           return redirect('admin/admin-users');
            ...
            Admin::where(['id' => $request->id])->update(['status' => 'Inactive']);
...
public function reply(Request $request)
{
  ...
  public function doc_index()
  {
		...
		$data['user']	    = DB::table('users')
								->orderBy('id','DESC')	
								->get();
        return view('admin.customers.doc_verification', $data);
  }
  
  public function approve_document_verification()
  {
		$data          		= [];
        $data['title'] 		= 'Document Verification';
		$data['user']	    = DB::table('users')
								->orderBy('id','DESC')	
								->get();
        return view('admin.customers.approve_doc_verification', $data);
  }
  ...
  {
	  DB::table('users_verification')->where('user_id',$id)->update(['document' => 'yes' ]);
	  return redirect('admin/document_verification');
  }
  
  public function disapprove_doc(Request $request)
  {
	  DB::table('users_verification')->where('user_id',$request->userid)->update(['document' => 'no', 'reason_for_disapprove' =>$request->reason_for_disapprove ]);
	  ...
	  $data = [
          ...
          'reason_for_disapprove'     => $request->reason_for_disapprove
        ];
        
        $email_controller = new EmailController;
        $email_controller->disapprove_user($data);
