...
class BookingsDataTable extends DataTable
{
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('payment_method', function ($bookings) {
                return $bookings->payment_method;
            })
    ...
    public function query()
    {
        ...
        $bookings = Bookings::join('properties', function ($join) {
                        ...
                        ->select(['bookings.payment_method', 'bookings.id as id', 'u.first_name as host_name', 'users.first_name as guest_name', 'bookings.property_id as property_id', 'properties.name as property_name', DB::raw('CONCAT(currency.symbol, bookings.total) AS total_amount'), 'bookings.status', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'bookings.service_charge', 'bookings.host_fee', 'bookings.iva_tax', 'bookings.accomodation_tax']);
        if (isset($user_id)) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'payment_method', 'name' => 'bookings.payment_method', 'title' => 'Payment method'])
