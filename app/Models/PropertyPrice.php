...
class PropertyPrice extends Model
{
    ...
    public function original_price($date)
    {
        ...
        if ($result->count()) {
            ...
            if ($dt->isWeekend()) {
                return $result->first()->weekend_price ?? $result->first()->price;
            }
            ...
            if ($dt->isWeekend()) {
                return $this->attributes['weekend_price'] ?? $this->attributes['price'];
            }
    ...
    {
        $where  = ['property_id' => $this->attributes['property_id'], 'date' => $date, 'min_stay' => 1];
        $result = PropertyDates::where($where);
    ...
    public function max_day($date)
    {
        $where  = ['property_id' => $this->attributes['property_id'], 'date' => $date, 'max_stay' => 1];
        $result = PropertyDates::where($where);

        if ($result->count()>0) {
            return $result->first()->max_day;
        } else {
            $max_day = 0;
            return $max_day;
        }
